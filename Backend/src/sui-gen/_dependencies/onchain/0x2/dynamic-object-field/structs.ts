...
export class Wrapper<T0 extends TypeArgument> implements StructClass { static readonly $typeName = "0x2::dynamic_object_field::Wrapper"; static readonly $numTypeParams = 1;
 ...
 private constructor(typeArgs: [ToTypeStr<T0>], fields: WrapperFields<T0>, ) { this.$fullTypeName = composeSuiType( Wrapper.$typeName, ...typeArgs ) as `0x2::dynamic_object_field::Wrapper<${ToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.name = fields.name; }

 static reified<T0 extends Reified<TypeArgument, any>>( T0: T0 ): WrapperReified<ToTypeArgument<T0>> { return { typeName: Wrapper.$typeName, fullTypeName: composeSuiType( Wrapper.$typeName, ...[extractType(T0)] ) as `0x2::dynamic_object_field::Wrapper<${ToTypeStr<ToTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [ToTypeStr<ToTypeArgument<T0>>], reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => Wrapper.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Wrapper.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => Wrapper.fromBcs( T0, data, ), bcs: Wrapper.bcs(toBcs(T0)), fromJSONField: (field: any) => Wrapper.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => Wrapper.fromJSON( T0, json, ), fromSuiParsedData: (content: SuiParsedData) => Wrapper.fromSuiParsedData( T0, content, ), fetch: async (client: SuiClient, id: string) => Wrapper.fetch( client, T0, id, ), new: ( fields: WrapperFields<ToTypeArgument<T0>>, ) => { return new Wrapper( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
