...
export class Referent<T0 extends TypeArgument> implements StructClass { static readonly $typeName = "0x2::borrow::Referent"; static readonly $numTypeParams = 1;
 ...
 private constructor(typeArgs: [ToTypeStr<T0>], fields: ReferentFields<T0>, ) { this.$fullTypeName = composeSuiType( Referent.$typeName, ...typeArgs ) as `0x2::borrow::Referent<${ToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.value = fields.value; }

 static reified<T0 extends Reified<TypeArgument, any>>( T0: T0 ): ReferentReified<ToTypeArgument<T0>> { return { typeName: Referent.$typeName, fullTypeName: composeSuiType( Referent.$typeName, ...[extractType(T0)] ) as `0x2::borrow::Referent<${ToTypeStr<ToTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [ToTypeStr<ToTypeArgument<T0>>], reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => Referent.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Referent.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => Referent.fromBcs( T0, data, ), bcs: Referent.bcs(toBcs(T0)), fromJSONField: (field: any) => Referent.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => Referent.fromJSON( T0, json, ), fromSuiParsedData: (content: SuiParsedData) => Referent.fromSuiParsedData( T0, content, ), fetch: async (client: SuiClient, id: string) => Referent.fetch( client, T0, id, ), new: ( fields: ReferentFields<ToTypeArgument<T0>>, ) => { return new Referent( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 id: this.id,value: fieldToJSON<Option<T0>>(`0x1::option::Option<${this.$typeArgs[0]}>`, this.value),
