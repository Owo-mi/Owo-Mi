...
export class VecSet<T0 extends TypeArgument> implements StructClass { static readonly $typeName = "0x2::vec_set::VecSet"; static readonly $numTypeParams = 1;
 ...
 private constructor(typeArgs: [ToTypeStr<T0>], fields: VecSetFields<T0>, ) { this.$fullTypeName = composeSuiType( VecSet.$typeName, ...typeArgs ) as `0x2::vec_set::VecSet<${ToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.contents = fields.contents; }

 static reified<T0 extends Reified<TypeArgument, any>>( T0: T0 ): VecSetReified<ToTypeArgument<T0>> { return { typeName: VecSet.$typeName, fullTypeName: composeSuiType( VecSet.$typeName, ...[extractType(T0)] ) as `0x2::vec_set::VecSet<${ToTypeStr<ToTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [ToTypeStr<ToTypeArgument<T0>>], reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => VecSet.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VecSet.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => VecSet.fromBcs( T0, data, ), bcs: VecSet.bcs(toBcs(T0)), fromJSONField: (field: any) => VecSet.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => VecSet.fromJSON( T0, json, ), fromSuiParsedData: (content: SuiParsedData) => VecSet.fromSuiParsedData( T0, content, ), fetch: async (client: SuiClient, id: string) => VecSet.fetch( client, T0, id, ), new: ( fields: VecSetFields<ToTypeArgument<T0>>, ) => { return new VecSet( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
